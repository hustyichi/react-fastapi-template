...
Introducing the React FastAPI Template: A cutting-edge foundation for modern full-stack development that seamlessly integrates React with FastAPI. This template combines Python's backend power with TypeScript's frontend, enhanced by Zod's robust type validation. Perfect for developers seeking a production-ready architecture that balances performance, type safety, and developer experience in one cohesive package.
...
## About
This template streamlines building APIs with [FastAPI](https://fastapi.tiangolo.com/) and dynamic frontends with [React](https://react.dev/) and [Vite](https://vitejs.dev/). It integrates the backend and frontend using [@hey-api/openapi-ts](https://github.com/hey-api/openapi-ts) to generate a type-safe client, with automated watchers to keep the OpenAPI schema and client updated, ensuring a smooth and synchronized development workflow.
...
Copy the `.env.example` files to `.env`. These values are unlikely to change, so you can leave them as they are.
   ```bash
   cd react-frontend && cp .env.example .env
   ```
...
#### Frontend
1. Navigate to the `react-frontend` directory and run:
   ```bash
   npm install
   ```
...
1. Start the FastAPI server container:
   ...
   ```
2. Start the React development server container:
   ```bash
...
- The **frontend hot reload** monitors changes to the frontend code and the `openapi.json` schema generated by the backend. Vite provides fast HMR (Hot Module Replacement) for React components.
...
2. To generate the frontend client:
   ```bash
   cd react-frontend && npm run generate-client
   ```
   or using Docker:
   ```bash
...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fvintasoftware%2Freact-fastapi-template%2Ftree%2Fmain%2Freact-frontend&env=VITE_API_BASE_URL&envDescription=The%20VITE_API_BASE_URL%20is%20the%20backend%20URL%20where%20the%20frontend%20sends%20requests.)
...
1. Link the react-frontend Project

2. Navigate to the react-frontend directory and run:
   ```bash
   cd react-frontend
   vercel link
   ```
...
4. Save Project IDs and Add GitHub Secrets:
  - Open `react-frontend/.vercel/project.json` and add the following to your GitHub repository secrets:
...
### Frontend
   ...
   - Update the `VITE_API_BASE_URL` variable with the backend URL once the backend deployment is complete.
